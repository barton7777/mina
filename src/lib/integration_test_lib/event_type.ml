...
module Gossip = struct
  ...
  module Snark_work = struct
    ...
    let id = Network_pool.Snark_pool.Resource_pool.Diff.snark_work_received_structured_events_id

    let parse_func message =
      match%bind parse id message with
      | Network_pool.Snark_pool.Resource_pool.Diff.Snark_work_received { work; sender = _ } ->
  ...
  module Transactions = struct
    ...
    let id = Network_pool.Transaction_pool.Resource_pool.Diff.transactions_received_structured_events_id

    let parse_func message =
      match%bind parse id message with
      | Network_pool.Transaction_pool.Resource_pool.Diff.Transactions_received { txns; sender = _ } ->
